...
const EMPTY_FORM = () => ({ desc:"", value:"", type:"expense", category:"outros", date:new Date().toISOString().slice(0,10), received:true });
...
export default function App() {
  ...
  const openEdit = (t) => {
    setEditingId(t.id);
    setForm({ desc:t.desc, value:String(t.value), type:t.type, category:t.category, date:t.date, received:t.received!==false });
    setShowForm(true);
  };
  ...
  return (<>
    <style>{`
      ...
      *,*::before,*::after{margin:0;padding:0;box-sizing:border-box;}
      ...
      .tx-meta{font-size:11px;color:var(--text3);margin-top:2px;display:flex;gap:8px;align-items:center;}
      ...
      .sav-result-card{border-radius:16px;padding:20px 22px;border:1px solid var(--border);background:var(--surface);margin-bottom:14px;}
        ...
        {view==="poupanca" && (<>
          ...
          {savModel==="custom" && (
            <div className="panel" style={{marginBottom:16}}>
              ...
              {(customSav.needs+customSav.wants+customSav.save)!==100 && (
                <div style={{marginTop:12,fontSize:11,color:"var(--red)",fontWeight:600}}>
                  ⚠ Total: {customSav.needs+customSav.wants+customSav.save}% (deve ser 100%)
                </div>
              )}
            </div>
          )}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            <div className="panel">
              ...
              {totalIncome===0
                ? <div className="empty">Adicione receitas confirmadas para calcular.</div>
                : (<>
                  {[
                  ...
                  <div style={{marginTop:16}}>
                    <SegmentBar segments={[
                      {color:"var(--blue)", pct:activeSav.needs},
                      {color:"var(--gold)", pct:activeSav.wants},
                      {color:"var(--green)",pct:activeSav.save},
                    ]}/>
            ...
            <div className="panel">
              ...
              {totalIncome===0
                ? <div className="empty">Sem receitas para comparar.</div>
                : (<>
                  {[
                  ...
                  <div style={{marginTop:16,padding:"12px 14px",borderRadius:12,background:saveDiff>=0?"rgba(109,232,160,0.06)":"rgba(232,122,109,0.06)",border:`1px solid ${saveDiff>=0?"rgba(109,232,160,0.2)":"rgba(232,122,109,0.2)"}`}}>
                    <div style={{fontSize:12,fontWeight:700,color:saveDiff>=0?"var(--green)":"var(--red)",lineHeight:1.5}}>
                      {saveDiff>=0
                        ? `🎉 Parabéns! Você está guardando ${fmtp(savePct)} da sua renda — ${fmt(saveDiff)} acima da meta.`
                        : `⚠ Você está guardando ${fmtp(savePct)} mas a meta é ${activeSav.save}%. Faltam ${fmt(Math.abs(saveDiff))} para atingir o ideal.`
                      }
                    </div>
                  </div>
                </>)
